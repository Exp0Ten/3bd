...
## GNU Binutils - readelf, objdump //maybe remove??

Tyto nástroje mi pomáhali při orientaci v informacích pro lazení, v struktuře spustitelných souborů *ELF* a v čtení *strojového kódu*. Díky těmto programům jsem se praktickou metodou naučil strukturu standardu *DWARF*.
...
Knihovna *nix* je nádstavbou Standardní knihovny pro jazyk C (známá také jako "libc"). Použil jsem ji hlavně pro moduly *ptrace*, *process* a *term*. *Ptrace* umožňuje sledování procesů, z *process* jsem použil funkci *fork* pro vytvoření nového podprocesu a *term* byl nezbytný pro vytvoření pseudoterminálu na standardní komunikaci.

### iced

Knihovna *iced* přináší jednoduché prostředí pro tvorbu grafických aplikacích. Ačkoliv je stále ve vývoji, nabízí bohatou škálu přehledných funkcí pro tvorbu grafiky a interakce s uživatelem. Taky jsem s ní s měl trochu zkušeností.

### gimli

Knihovna *gimli* spravuje informace pro lazení, zejména ty pod standardem *DWARF*. Jedná se o velmi silný modul na *parsování* těchto dat. Používám ji pro zobrazování *zdrojového kódu*,  a sledování lokálních proměnných.
